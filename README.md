...
See section [Searching](#searching) for more details on the search behaviour.

### getDefaultSorting
expected result: all properties plus the wanted direction that shall be sorted by default whenever allExtended is
called.

See section [Sorting](#sorting) for more details on the sorting behavior.
...
[HTTP List request options and parameters](#HTTP List request options and parameters) for further detail on the
...
###### $preSetFilters (second parameter)
...
###### $preSetOrFilters (third parameter)
...
###### $preSetIncludes (fourth parameter)
...
###### $preSetSearches (fifth parameter)
...
###### $preSetOrSearches (sixth parameter)
...
### HTTP List request options and parameters
